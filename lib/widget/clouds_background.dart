...
class Clouds extends AnimatedWidget {
  ...
  Widget build(BuildContext context) {
    ...
    var _paintBrush = Paint()/// takes no arguments but has setters to change paints'qlty
    ...
    return Container(
      height: 300.0,
      child: CustomPaint(/// canvas wrapped must be wrapped in CustomPaint widget
        size: screenSize,
...
class CloudPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    var rectTop = 110.0;/// rounded rectangle
    var rectBottom = rectTop + 40.0;

    /// variables to position the cloud on the horizontal axis
    var figureLeftEdge = size.width / 4;
    var figureRightEdge = size.width - 90.0;
    var figureCenter = size.width / 2;

    var cloudBaseRect = Rect.fromPoints(/// create rectangle from top left and bottom right
      /// offsets
      Offset(figureLeftEdge, rectTop),
      Offset(figureRightEdge, rectBottom),
    );

    var cloudBase = RRect.fromRectAndRadius(/// rectangle with rounded corners
      cloudBaseRect,
      Radius.circular(10.0),
    );

    /// executing the drawing of circles
    ...
    canvas.drawRRect(cloudBase, cloudPaint);/// the rectangle is finally drawn here
