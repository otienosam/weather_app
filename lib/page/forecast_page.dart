...
class _ForecastPageState extends State<ForecastPage> with TickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(ForecastPage oldWidget) {// ensures the animation is fired when
  ...
  @override
  void dispose() {
    _animationController?.dispose();// disposes the animation when its done
    _weatherConditionAnimationController?.dispose();
    super.dispose();
  }

  void _render() {
    _forecastController.city = widget.settings.activeCity;// method for page configuration
    // settings
    ...
    );// tells the object of the current animation's values
  ...
  void _handleDragEnd(DragUpdateDetails d, BuildContext context) {
    ...
    var dragEnd = d.globalPosition.dy;//position on the screens y-axis where pointer was
    //located
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: GestureDetector(
        ...
        onVerticalDragUpdate: (DragUpdateDetails details) {//gesture to provide details
          _handleDragEnd(details, context);
        },
        child: ColorTransitionBox(//widget responds to gestures
          animation: _backgroundColorTween.animate(_animationController),
          child: Padding(
            padding: EdgeInsets.symmetric(vertical: 32.0),
            child: Stack(
              children: <Widget>[
                SlideTransition(///moon or sun background painting used instead of positioned
                  position: _positionOffsetTween.animate(//position value can change
                    _animationController.drive(///method to change controller's default
                      CurveTween(curve: Curves.bounceOut),/// trajectory is changed
                    ),
                  ),
                  child: Sun(
                ...
                SlideTransition(
                  ...
                  child: Clouds( //clouds overlaps the sun
                    isRaining: isRaining,
                    animation: _cloudColorTween.animate(_animationController),
                  ),
                ),
                Column(//topmost layer in the stack containing the content
                  verticalDirection: VerticalDirection.up,//reverses the direction of
                  // the column
                  children: <Widget>[
                    forecastContent,//variable of the table widget
                    mainContent,
